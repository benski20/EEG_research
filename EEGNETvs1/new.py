# embedding_constrained_architecture.py - FIXED VERSION
...
def analyze_word_categories(words, embeddings_dict):
    ...
    # Simple clustering to identify word categories
    print(f"✅ Found {len(valid_embeddings)} valid embeddings for architecture design")
    return {"num_clusters": 3, "valid_embeddings": len(valid_embeddings)}

# -------------------------
# SPECIALIZED EEG PROCESSING BRANCHES - FIXED DIMENSIONS
# -------------------------
...
def main():
    ...
    model = Model(inputs=eeg_input, outputs=[emotion_logits, pos_logits], 
                  name="upd_embedding_constrained_architecture")
    ...
    callbacks = [
        ...
        ModelCheckpoint(filepath='upd_embedding_constrained_model.keras', monitor='val_loss', 
    ...
    # Evaluation
    print("📊 Evaluation...")
    # Create custom_objects for loading
    custom_objects = {
        'focal_loss_fixed': balanced_focal_loss(),
        'AdaptiveEEGProcessor': AdaptiveEEGProcessor,
        'EmotionOptimizedEEGBranch': EmotionOptimizedEEGBranch,
        'ConcreteOptimizedEEGBranch': ConcreteOptimizedEEGBranch,
        'AbstractOptimizedEEGBranch': AbstractOptimizedEEGBranch
    }
    
    best_model = load_model('upd_embedding_constrained_model.h', custom_objects=custom_objects)
